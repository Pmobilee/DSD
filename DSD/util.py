...
@torch.no_grad()
def compare_teacher_student(teacher, sampler_teacher, student, sampler_student, steps=[10], prompt=None, total_steps=64):
    """
    ...
    with torch.no_grad():
        # with teacher.ema_scope():
            for sampling_steps in steps:
                ...
                xc = torch.tensor([class_image])
                c = teacher.get_learned_conditioning({teacher.cond_stage_key: xc.to(teacher.device)})
                samples_ddim_teacher, teacher_intermediate, x_T_copy, pred_x0_teacher, a_t_teacher, v_teach= sampler_teacher.sample(S=sampling_steps,
                                                    conditioning=c,
                                                    batch_size=1,
                ...
                # x_samples_ddim = teacher.decode_first_stage(_["pred_x0"][-1)
                x_samples_ddim = teacher.decode_first_stage(samples_ddim_teacher)
                x_samples_ddim = torch.clamp((x_samples_ddim+1.0)/2.0, min=0.0, max=1.0)
                images.append(x_samples_ddim)
                # with student.ema_scope():
                sc = student.get_learned_conditioning({student.cond_stage_key: torch.tensor(1*[1000]).to(student.device)})
                c = student.get_learned_conditioning({student.cond_stage_key: xc.to(student.device)})
                samples, teacher_intermediate, _, pred_x0_student, a_t_teacher, v_student = sampler_student.sample(S=sampling_steps,
                                                    conditioning=c,
                                                    batch_size=1,
...
def get_fid(model, sampler, num_imgs, name,instance, steps =[4, 2, 1], x_0=False):
    ...
    with torch.no_grad():
        ...
        for step in steps:
            ...
            f"{cwd}/saved_images/cin/FID/{name}/{instance}/{step}"], batch_size = 16, device='cuda', dims=2048)
            fid_list.append(fid)
    return fid_list
