...
class DDIMSampler(object):
    ...
    def p_sample_ddim(self, x, c, t, index, repeat_noise=False, use_original_steps=False, quantize_denoised=False,
        ...
        a_t = torch.full((b, 1, 1, 1), alphas[index], device=device, requires_grad=True)
        a_prev = torch.full((b, 1, 1, 1), alphas_prev[index], device=device, requires_grad=True)
    ...
    def sample_student(self,
        ...
        size = (batch_size, C, H, W)
        # print(f'Data shape for DDIM sampling is {size}, eta {eta}')
        samples, intermediates, x_T_copy = self.ddim_sampling_student(conditioning, size,
                                                    callback=callback,
                                                    img_callback=img_callback,
                                                    quantize_denoised=quantize_x0,
                                                    mask=mask, x0=x0,
