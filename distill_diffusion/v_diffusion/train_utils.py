...
class Trainer:
            ...
            with tqdm(
                ...
                for i, (x, y) in enumerate(t):
                    ...
                    # if total_batches % 5000 == 0:
                        ...
                        # noises = torch.randn((self.num_save_images, ) + self.shape)
                        eval_results, fid = evaluator.eval(self.sample_fn, noises=noises, labels=labels, shape=self.shape, num_classes= self.num_classes, random_labels= self.random_labels, step = total_batches, max_eval_count=5000)
                        # eval_results, fid = evaluator.eval(self.sample_fn, noises=noises, labels=labels, max_eval_count=5000)y
                        if session != None:
    ...
    def FID(self, evaluator, name):
        ...
        # if self.is_main and self.num_save_images:
            ...
            eval_results, fid = evaluator.eval(self.sample_fn, noises=noises, labels=labels, max_eval_count=10000)
...
class Evaluator:
    ...
    # def eval(self, sample_fn, noises, labels, max_eval_count=10000, timesteps=128, base_folder_path="FID_IMAGES_calc"):
    ...
    #     # Find the highest numbered folder and create a new one
    #     existing_folders = [f for f in os.listdir(base_folder_path) if os.path.isdir(os.path.join(base_folder_path, f))]
    #     highest_folder_num = max([int(f) for f in existing_folders] + [0])  # Default to 0 if no folders exist
    #     new_folder_num = highest_folder_num + 1
    #     folder_path = os.path.join(base_folder_path, str(new_folder_num))
    ...
    def eval(self, sample_fn, noises, labels, shape, num_classes, random_labels, step, max_eval_count=10000, timesteps=128, base_folder_path="FID_IMAGES_calc"):
        base_folder_path = base_folder_path + "/" + str(self.dataset) 
        # Ensure base folder exists
        if not os.path.exists(base_folder_path):
            os.makedirs(base_folder_path)
        ...
        # highest_folder_num = max([int(f) for f in existing_folders] + [0])  # Default to 0 if no folders exist
        # new_folder_num = highest_folder_num + 1
        folder_path = os.path.join(base_folder_path, str(step))
        os.makedirs(folder_path)

        for i in range(max_eval_count):
            # Generate new noise and label for each image
            noise = torch.randn((1,) + shape)
            if num_classes > 0:
                label = random_labels()
            else:
                label = None
            ...
            # Generate image
            x = sample_fn(noises=noise, labels=label, use_ddim=True, timesteps=timesteps)

            # Save image
            save_image(x, os.path.join(folder_path, f"image_{i + 1}.jpg"))
